...
export class WorkspaceNode extends vscode.TreeItem {
  ...
  constructor(
    ...
    super(workspaceInfo.Path, vscode.TreeItemCollapsibleState.Expanded);
    this.contextValue = 'ring.workspace';
    this.iconPath = WorkspaceNode.getIconPath(
      workspaceInfo.WorkspaceState,
      context
    );
    this.children = workspaceInfo.Runnables.map(
      (r) => new RunnableNode(r, context)
    );
...
export class RunnableNode extends vscode.TreeItem {
  ...
  constructor(
    ...
    this.contextValue =
      ',' +
      Object.keys(runnable.Details).reduce((m, x) => m + ',' + x, '') +
      ',';
...
export class WorkspaceProvider implements vscode.TreeDataProvider<RingNode> {
  private _onDidChangeTreeData: vscode.EventEmitter<RingNode | null> =
    new vscode.EventEmitter<RingNode | null>();
  readonly onDidChangeTreeData: vscode.Event<RingNode | null> =
    this._onDidChangeTreeData.event;
  private root: WorkspaceNode;

  public static emptyWorkspace: IWorkspaceInfo = {
    Runnables: [],
    Flavours: [],
    CurrentFlavour: '',
    Path: 'Not loaded',
  ...
  getChildren(
    element?: RingNode | undefined
  ): vscode.ProviderResult<RingNode[]> {
    if (element) {
  ...
  getRunnable(id: string) {
    return this.root.children.find((r) => r.id === id);
  }
  ...
  resetWorkspace() {
    this.root = new WorkspaceNode(
      WorkspaceProvider.emptyWorkspace,
      this.context
    );
  }
